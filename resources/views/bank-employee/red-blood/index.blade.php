...
<head>
    ...
    <title>Red Blood Inventory</title>
    ...
    <main class="table">
            ...
            <section class="table__body">
                <table>
                    ...
                    <tbody>
                        @foreach($records as $redBlood)
                        <tr>
                            <td>{{ $redBlood->id }}</td>
                            <td>{{ $redBlood->blood_type }}</td>
                            <td>{{ $redBlood->rhesus_factor }}</td>
                            <td>{{ $redBlood->quantity }}</td>
                            <td>{{ \Carbon\Carbon::parse($redBlood->original_date)->format('Y-m-d') }}</td>
                            <td>
                                {{ \Carbon\Carbon::now()->diffInDays($redBlood->expiration_timer, false) }} days left
                            </td>
                            <td>
                                <a href="{{ route('bank-employee.red-blood.edit', $redBlood->id) }}" class="info-btn" style="background: #28a745;">Edit</a>
                                <form action="{{ route('bank-employee.red-blood.destroy', $redBlood->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
