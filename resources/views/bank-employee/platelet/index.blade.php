...
<head>
    ...
    <title>Platelet Inventory</title>
    ...
    <main class="table">
            ...
            <section class="table__body">
                <table>
                    ...
                    <tbody>
                        @foreach($records as $platelet)
                        <tr>
                            <td>{{ $platelet->id }}</td>
                            <td>{{ $platelet->blood_type }}</td>
                            <td>{{ $platelet->rhesus_factor }}</td>
                            <td>{{ $platelet->quantity }}</td>
                            <td>{{ \Carbon\Carbon::parse($platelet->original_date)->format('Y-m-d') }}</td>
                            <td>
                                {{ \Carbon\Carbon::now()->diffInDays($platelet->expiration_timer, false) }} days left
                            </td>
                            <td>
                                <a href="{{ route('bank-employee.platelet.edit', $platelet->id) }}" class="info-btn" style="background: #28a745;">Edit</a>
                                <form action="{{ route('bank-employee.platelet.destroy', $platelet->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
